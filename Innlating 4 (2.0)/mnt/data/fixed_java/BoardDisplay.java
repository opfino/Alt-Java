public class BoardDisplay {
    ...
    private boolean[][] northWalls; // optional, kept so your NorthWall class still works
    private boolean[][] westWalls;  // optional, kept so your WestWall class still works
    private int rows;
    private int cols;
    ...
    public void setNorthWall(int x, int y) {
        if (isInside(x, y)) {
            northWalls[y][x] = true;
        }
    }

    public void setWestWall(int x, int y) {
        if (isInside(x, y)) {
            westWalls[y][x] = true;
        }
    }
    ...
    public void show() {
        ...
        for (int y = 0; y < rows; y++) {
            ...
            for (int x = 0; x < cols; x++) {
                ...
                boolean hasWestWallOfNextCell = x + 1 < cols && westWalls[y][x + 1];
                System.out.print((hasEastWall || hasWestWallOfNextCell) ? "|" : " ");
            }
            System.out.println();
